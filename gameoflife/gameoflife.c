...
...data is here...
  ...
  fprintf(fp, "<Piece Extent=\"%d %d %d %d 0 0\">", xStart, xEnd, yStart, yEnd);
...
void writeVTK2Container(long timestep, double *data, char prefix[1024], long w, long h, int *area_bounds, int num_threads) {
  ...
  fprintf(fp,"<PCellData Scalars=\"cell_scalars\">");
  fprintf(fp,"<PDataArray type=\"Float64\" Name=\"cell_scalars\"/>");
  fprintf(fp,"</PCellData>");

  for(int i = 0; i < num_threads; i++) {
    fprintf(fp, "<Piece Extent=\"%d %d %d %d 0 0\" Source=\"%s-%05ld-%02d%s\"/>",
      area_bounds[i * 4], area_bounds[i * 4 + 1], area_bounds[i * 4 + 2], area_bounds[i * 4 + 3], prefix, timestep, i, ".vti");
  }

  fprintf(fp,"</PImagData>");
  fprintf(fp, "</VTKFile>\n");
  fclose(fp);

  /*<?xml version=\"1.0"?>
<VTKFile type=\"PImageData" version=\"0.1" byte_order=\"LittleEndian">
 <PImageData WholeExtent=\"0 640 0 640 0 1" Origin=\"0 0 0 " Spacing = "1 1 1" GhostLevel=\"0">
